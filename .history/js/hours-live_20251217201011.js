/* =========================================================
  Öffnungszeiten: "Heute" + Live-Status (premium, ohne перегруза)
  - Работает для любого числа блоков на странице
  - Каждый блок должен иметь data-hours-root
========================================================= */
(function () {
  "use strict";
  ...
  function getWeekdayMo1(){ const jsDay=new Date().getDay(); return jsDay===0?7:jsDay; }
  ...
  function normalizeWeekday(v){
    ...
    const map={ mo:1,montag:1, di:2,dienstag:2, mi:3,mittwoch:3, do:4,donnerstag:4, fr:5,freitag:5, sa:6,samstag:6, so:7,sonntag:7 };
    return map[s.toLowerCase()] || null;
  }

  function parseRanges(str){
    if(!str) return [];
    return str.split(",").map(x=>x.trim()).filter(Boolean).map((r)=>{
  ...
  function setHeuteBadge(dayEl){
    ...
    b.className="today-badge ml-2 inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-700 ring-1 ring-emerald-100";
    b.textContent="Heute";
    dayEl.appendChild(b);
  }

  // premium: kurz + Details in title
  function setLiveBadge(targetEl, text, variant, titleText){
    const b=document.createElement("span");
    ...
    else b.classList.add("bg-white/70","text-slate-700","ring-slate-200");
  ...
  function initInScope(scope){
    ...
    function findNextOpen(today, minsNow){
      for(let off=1; off<=7; off++){
        ...
        if(rr.length) return { day:d, range:rr[0], offsetDays:off, minsNow };
      }
      return null;
    }
    ...
    function render(){
      ...
      if(!ranges.length){
        ...
        if(next){
          ...
          setLiveBadge(timeWrap, `öffnet ${label} ${next.range.sTxt}`, "closed", `In ${fmtDuration(minsTo)}`);
        } else {
      ...
      if(openRange){
        setLiveBadge(timeWrap, `noch ${fmtDurationShort(openRange.e-minsNow)}`, "open", `Geöffnet · bis ${openRange.eTxt}`);
        return;
      }
      ...
      if(nextToday){
        setLiveBadge(timeWrap, `öffnet ${nextToday.sTxt}`, "closed", `In ${fmtDuration(nextToday.s-minsNow)}`);
        return;
      }
  ...
  function boot(){
    // Главное: каждый блок отдельно
    const roots = Array.from(document.querySelectorAll("[data-hours-root]"));

    // совместимость со старыми id (если забыл data-hours-root)
    if(!roots.length){
    ...
    if(!roots.length){
      console.warn("[hours-live] Нет контейнера с data-hours-root. Добавь data-hours-root к блоку Öffnungszeiten.");
      return;
    }
